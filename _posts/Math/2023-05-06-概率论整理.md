...
|均匀分布$X\sim U(a,b)$|$f(x) = \begin{cases}\frac{1}{b-a},&a<x<b\\0,&其他\end{cases}$|$\frac{a+b}{2}$|$\frac{(b-a)^2}{12}$|$-\infty<a<b<+\infty$|
|指数分布$X\sim E(\lambda)$|$f(x) = \begin{cases}\lambda e^{-\lambda x},&x>0\\0,&x\leq 0\end{cases}$|$\frac{1}{\lambda}$|$\frac{1}{\lambda^2}$|$\lambda>0$|
...
2. $\ln L(\theta) = \sum_{i=1}^{n}\ln f(x_i;\theta)$
3. $\frac{\partial \ln L(\theta)}{\partial \theta} = 0$
4. $\frac{\partial^2 \ln L(\theta)}{\partial \theta^2} < 0$
5. 
